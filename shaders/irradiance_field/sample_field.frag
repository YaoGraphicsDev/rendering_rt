...
#define VIEW_FROM_CAMERA    0
#define VIEW_FROM_PROBE     1
layout (push_constant) uniform PushConstants {
    vec4 projEncoded;
    vec4 viewBaseQuat;
    vec3 camPos;
    DDGIParams ddgi_params;
    float normalBias;
    uint sampleAtlasIndex; // 0, 1
    uint viewType; // 0 -- from camera, 1 -- from probe
} consts;
...
//  binding 16 - 23:    SSBO
layout(set = 3, binding = 0) uniform texture2D texPosition;
layout(set = 3, binding = 1) uniform texture2D texNormal;
layout(set = 3, binding = 2) uniform texture2D texAlbedo;
...
layout(set = 3, binding = 8) uniform texture2D texRayOrigins; // only relevant when viewType == VIEW_FROM_PROBE
...
ivec3 baseGridCoord(vec3 p) {
    vec3 grid = (p - consts.ddgi_params.probeStart) / consts.ddgi_params.probeStep;
...
void main() {
    ...
    // in world space
    vec3 view = vec3(0.0);
    vec3 position = texelFetch(texPosition, pixel, 0).xyz;
    vec3 normal  = normalize(texelFetch(texNormal, pixel, 0).xyz);
    if (consts.viewType == VIEW_FROM_CAMERA) {
        view = normalize(consts.camPos - position);
    } else if (consts.viewType == VIEW_FROM_PROBE) {
        view = normalize(texelFetch(texRayOrigins, pixel, 0).xyz - position);
    } else {
        result = vec4(1.0f, 0.0f, 1.0f, 1.0f); // magenta upon error
        return;
    }

    ivec3 baseGridCoord = baseGridCoord(position);
    if (any(greaterThanEqual(baseGridCoord, consts.ddgi_params.probeCounts - ivec3(1)))) {
    ...
    vec3 baseProbePos = gridCoordToPosition(baseGridCoord, consts.ddgi_params.probeStart, consts.ddgi_params.probeStep);
    vec3 sumIrradiance = vec3(0.0);
    float sumWeight = 0.0;

    // alpha is how far from the floor(currentVertex) position. on [0, 1] for each axis.
    vec3 alpha = clamp((position - baseProbePos) / consts.ddgi_params.probeStep, vec3(0.0), vec3(1.0));

    for (int i = 0; i < 8; ++i) {
        // Compute the offset grid coord and clamp to the probe grid boundary
        // Offset = 0 or 1 along each axis
        ivec3 offset = ivec3(i, i >> 1, i >> 2) & ivec3(1);
        ivec3 probeGridCoord = clamp(baseGridCoord + offset, ivec3(0), ivec3(consts.ddgi_params.probeCounts - 1));
        int probeID = gridCoordToProbeIndex(probeGridCoord, consts.ddgi_params.probeCounts);

        vec3 probePos = gridCoordToPosition(probeGridCoord, consts.ddgi_params.probeStart, consts.ddgi_params.probeStep);
        ...
        vec3 fromProbeBiased = position - probePos + (normal + 3.0 * view) * consts.normalBias;
        vec3 dirFromProbeBiased = normalize(fromProbeBiased);
        {
            vec2 texCoord = textureCoordFromDirection(dirFromProbeBiased, probeID, consts.ddgi_params.depthAtlasSize, consts.ddgi_params.depthProbeSize);
        ...
        vec2 texCoord = textureCoordFromDirection(normal, probeID, consts.ddgi_params.irradAtlasSize, consts.ddgi_params.irradProbeSize);
    ...
    vec3 albedo = texelFetch(texAlbedo, pixel, 0).rgb;
    vec2 metallicRoughness = texelFetch(texMetallicRoughness, pixel, 0).rg;
